...
#' @param gene.set Reference gene set
#' @param class.name the name of the class / variable to which genes have been correlated (e.g. drug-treatment)
#' @param metric.range the range of the metric; defaults to [min(DEFINED RANGE), max(DEFINED RANGE)]
#' @param enrichment.score.range the range of the enrichment score; defaults to [min(ENRICHMENT SCORE), max(ENRICHMENT SCORE)]
#'
#' @return None
#' @export
...
replotGSEA <- function(path, gene.set, class.name, metric.range,
  ...
  plot(c(1, gsea.hit.indices, length(gsea.rnk$metric)),
       c(0, gsea.es.profile, 0), type = "l", col = "red", lwd = 3.5, xaxt = "n", # Camilo changed lwd of 1.5 to 3.5
       xaxs = "i", xlab = "", ylab = "Enrichment score (ES)", cex.lab = 1.5, # Camilo added cex.lab = 1.5
       ylim = enrichment.score.range,
       main = list(gsea.gene.set, font = 2, cex = 2), # Camilo changed font to bold (2) and cex to 2
       panel.first = {
  ...
  if(gsea.enrichment.score < 0) {
    text(length(gsea.rnk$metric) * 0.01, plot.coordinates[3] * 0.98,
         paste("Nominal p-value:", gsea.p.value, "\nFDR:", gsea.fdr, "\nES:",
               ...
               gsea.normalized.enrichment.score), adj = c(0, 0), cex = 1.5) # Camilo added cex = 1.5
  } else {
    text(length(gsea.rnk$metric) * 0.99, plot.coordinates[4] - ((plot.coordinates[4] - plot.coordinates[3]) * 0.03),
         paste("Nominal p-value:", gsea.p.value, "\nFDR:", gsea.fdr, "\nES:",
               gsea.enrichment.score, "\nNormalized ES:",
               gsea.normalized.enrichment.score, "\n"), adj = c(1, 1), cex = 1.5) # Camilo added cex = 1.5
  }
  ...
  text(length(gsea.rnk$metric) / 2, 0.7,
       labels = ifelse(!missing(class.name), class.name, gsea.template), cex = 1.5) # Camilo added cex = 1.5
